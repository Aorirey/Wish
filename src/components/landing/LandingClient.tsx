...
function HeroFloats({ items }: { items: FloatItem[] }) {
  return (
    <div
      aria-hidden
      ...
      <div className="relative overflow-hidden rounded-3xl border border-ink-200 dark:border-ink-700 bg-gradient-to-br from-ink-950 via-ink-900 to-ink-800 p-10 text-white sm:p-16">
        ...
        <div className="relative max-w-xl">
          ...
          <p className="mt-4 text-ink-200">
            В каталоге уже {products.toLocaleString("ru-RU")} товаров из{" "}
            {stores} российских магазинов. Бесплатно, навсегда, без алгоритмов, которые пытаются продать вам чувства.
          </p>
          <div className="mt-8 flex flex-col items-start gap-3 sm:flex-row sm:items-center">
            ...
            <span className="text-xs text-ink-300">
              Регистрация не требуется — можно просто попробовать.
            </span>
          </div>
        </div>
      </div>
...
export function LandingClient({
  ...
  friendAvatars: string[];
  stats: { products: number; stores: number; adds: number };
}) {
  return (
    <div className="relative overflow-hidden">
      ...
      <CTA products={stats.products} stores={stats.stores} />
      <Footer />
    </div>
  );
}
